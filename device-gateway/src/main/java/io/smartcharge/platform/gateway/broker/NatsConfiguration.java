...
class NatsConfiguration {
    ...
    private static void ensureStream(JetStreamManagement management, String name, String subject, Duration maxAge)
        ...
        try {
            ...
            try {
                management.addStream(StreamConfiguration.builder().name(name).subjects(subject)
                        .storageType(StorageType.File).maxAge(maxAge).duplicateWindow(Duration.ofMinutes(10)).build());
