...
const deviceForm = reactive({ stationId: '', deviceCode: '', protocolCode: '', productModel: '', connectorCount: 12, ratedPowerW: 800 })
const tariffForm = reactive({ name: '', billingMode: 'DURATION', durationUnitPriceMinor: 1, energyUnitPriceMinor: 0, minimumAmountMinor: 1 })
...
const merchantForm = reactive({ channel: 'WECHAT', merchantId: '', applicationId: '', secretReference: 'env:WECHAT_PRIMARY', notifyUrl: '', refundNotifyUrl: '', status: 'ACTIVE' })
...
async function createDevice() { await refresh('assets', () => postJson('/admin/assets/devices', { ...deviceForm, firmwareVersion: null }), '设备及端口已创建'); Object.assign(deviceForm, { stationId: '', deviceCode: '', protocolCode: '', productModel: '', connectorCount: 12, ratedPowerW: 800 }) }
...
async function transitionWorkOrder(row: Row, status: string) { await refresh('operations', () => postJson(`/admin/operations/work-orders/${row.id}/transition`, { status, assigneeSubject: row.assigneeSubject || 'operator', version: row.version }), '工单状态已更新') }
