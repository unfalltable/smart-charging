...
| 设备 TCP 网关 | `localhost:9000` |
| 网关健康检查 | `http://localhost:9001` |
| PostgreSQL | `localhost:5432` |
| Valkey | `localhost:6379` |
| NATS / 监控 | `localhost:4222` / `http://localhost:8222` |
...
刷新 `http://localhost:8088`，即可在设备、订单、财务和审计页面查看结果。本地演示模式不要求 OIDC 登录，也不会调用真实微信支付。
...
2. 本地开发配置默认请求 `http://127.0.0.1:8080/api/v1`，项目已关闭开发工具的域名校验。
3. 点击“我的 → 登录”，本机 `local` profile 会返回测试用户会话。
4. 使用启动脚本输出的二维码内容制作二维码后扫码，或直接用 `ops/pilot-create-order.http` 调接口。

真机预览时，手机里的 `127.0.0.1` 指向手机自身。需要把 `apps/miniapp/src/config.js` 的 `develop.apiBase` 改为电脑的局域网 IPv4 地址，并允许 Windows 防火墙访问对应端口。体验版和正式版的地址仍是无效占位域名，发布前必须替换为已备案且配置 HTTPS 的真实 API 域名。
